...
function saveLikedProfile() {
  ...
  const profileIndex = likedProfiles.findIndex(
    (profile) => profile.name.first === currentProfile.name.first
  );

  if (profileIndex !== -1) {
    // Erstatter eksisterende profil
    likedProfiles.splice(profileIndex, 1, currentProfile);
  } else {
    // Legger til ny profil
